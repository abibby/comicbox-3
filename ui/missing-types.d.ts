...
declare module '@surma/rollup-plugin-off-main-thread' {
    export interface Options {
        // A string containing the EJS template for the amd loader. If undefined, OMT will use loader.ejs.
        loader?: string
        // Use fetch() + eval() to load dependencies instead of <script> tags and importScripts(). This is not CSP compliant, but is required if you want to use dynamic imports in ServiceWorker.
        useEval?: boolean
        // A RegExp to find new Workers() calls. The second capture group must capture the provided file name without the quotes.
        workerRegexp?: RegExp
