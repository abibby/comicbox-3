...
function Alert<T>(props: AlertProps<T>) {
    const buttons = Object
        ...
        .map(([name, result]) => <button class={styles.option} onClick={bind(result, props.close)}>{name}</button>)
...
export async function prompt<T>(message: string, options: Record<string, T>, timeout = 5000): Promise<T | undefined> {    
